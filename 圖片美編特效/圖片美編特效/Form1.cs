...
namespace 圖片美編特效
{
    public partial class Form1 : Form
    {
        ...
        {
            //button16.Image = pictureBox1.BackgroundImage;
        }
        ...
        private void toolStripMenuItem3_Click(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                IntPtr image = CvInvoke.cvLoadImage(
                    filename,
                    Emgu.CV.CvEnum.LOAD_IMAGE_TYPE.CV_LOAD_IMAGE_COLOR);
        ...
        private void toolStripMenuItem5_Click(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                pictureBox1.Image.Save(filenameqq);
            }
        }
        ...
        {
            size = CvInvoke.cvGetSize(ju);
            dest = new Image<Bgr, byte>(size);
            CvInvoke.cvCopy(ju, dest, IntPtr.Zero);
            for (int i = 0; i < dest.Height; i += 10)
            {
                for (int j = 0; j < dest.Width; j++)
                {
                    dest.Data[i, j, 0] = (byte)255;
                    dest.Data[i, j, 1] = (byte)0;
                    dest.Data[i, j, 2] = (byte)0;
                }
            }
            pictureBox1.Image = dest.ToBitmap(); 

            //button16.Text = pictureBox1.Name;
            //button16.Image = pictureBox1.Image;
